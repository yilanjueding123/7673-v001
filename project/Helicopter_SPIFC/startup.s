    ...
    IMPORT 	|Load$$ROM3$$Base|				; iRAM region
    IMPORT 	|Image$$ROM3$$Base|
    IMPORT 	|Image$$ROM3$$Length|
    ;IMPORT 	|Load$$ROM5$$Base|				; iRAM region
    ;IMPORT 	|Image$$ROM5$$Base|
    ;IMPORT 	|Image$$ROM5$$Length|
...
213
	...
	CMP		R1, R2
	BEQ		%F233								; If load address is equal to execution address, movement is not needed
    CMP     R3, #0x0
    BLE     %F233
223
    LDR     R4, [R1], #4
...
;215
;    LDR     R1, =|Load$$ROM5$$Base|
;    LDR     R2, =|Image$$ROM5$$Base|
;    LDR     R3, =|Image$$ROM5$$Length|
;	CMP		R1, R2
;	BEQ		%F235								; If load address is equal to execution address, movement is not needed
;    CMP     R3, #0x0
;    BLE     %F235
;225
;    LDR     R4, [R1], #4
